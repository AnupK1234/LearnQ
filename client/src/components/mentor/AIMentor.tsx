...
const AIMentor = () => {
  ...
  const mentors = {
    einstein: {
      ...
      description:
        "Explains complex concepts with thought experiments and analogies.",
      color: "bg-blue-500",
      greeting: {
    ...
    if (mentorKey !== selectedMentor) {
      ...
      toast.success(
        `You are now speaking with ${
          mentors[mentorKey as keyof typeof mentors].name
        }!`
      );
    } else {
      setShowMentorSelection(false);
  ...
  const simulateMentorResponse = (question: string) => {
    ...
    setTimeout(() => {
      ...
      if (
        lowercaseQuestion.includes("quantum") ||
        lowercaseQuestion.includes("physics")
      ) {
        response =
          sampleResponses["Can you help me understand quantum physics?"];
      } else if (
        lowercaseQuestion.includes("research") ||
        lowercaseQuestion.includes("paper") ||
        lowercaseQuestion.includes("write")
      ) {
        response = sampleResponses["How do I write a good research paper?"];
      } else if (
        lowercaseQuestion.includes("motivation") ||
        lowercaseQuestion.includes("struggling") ||
        lowercaseQuestion.includes("study")
      ) {
        ...
        response = `That's an interesting question about ${question
          .split(" ")
          .slice(0, 3)
          .join(" ")}. As a specialist in ${
          mentorData.specialty
        }, I would approach this by first understanding the fundamental concepts, then building up to more complex ideas. Let me walk you through how I would think about this...`;
      }
          ...
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6">
              <div className="relative">
                <Avatar
                  className={`h-24 w-24 rounded-xl ${currentMentor.color}`}
                >
                      ...
                      <div className="p-2">
                        {Object.entries(mentors).map(([key, mentor]) => (
                          <Button
                            key={key}
                            variant={
                              key === selectedMentor ? "secondary" : "ghost"
                            }
                            className="w-full justify-start mb-1 last:mb-0"
                            onClick={() => handleSelectMentor(key)}
                          >
                            <div
                              className={`w-3 h-3 rounded-full ${mentor.color} mr-2`}
                            ></div>
